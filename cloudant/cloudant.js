...
let debug = require('debug')('cloudant:cloudant');
let nanodebug = require('debug')('nano');

const Client = require('./lib/client.js');

// Parse an object (i.e. { account: "myaccount", password: "mypassword" }) and
// return a URL.
let reconfigure = require('./lib/reconfigure.js');
...
function Cloudant(options, callback) {
  ...
  let cloudantClient = new Client(options);
  let cloudantRequest = function(req, callback) {
  ...
  let use = function(db) {
    ...
    let bulk_get = function(options, callback) { // eslint-disable-line camelcase
    ...
    let geo = function(docName, indexName, query, callback) {
      let path = encodeURIComponent(db) + '/_design/' +
    ...
    let get_security = function(callback) { // eslint-disable-line camelcase
      let path = '_api/v2/db/' + encodeURIComponent(db) + '/_security'; // eslint-disable-line camelcase
    ...
    let set_security = function(permissions, callback) { // eslint-disable-line camelcase
      let body = permissions;
      let prefix = '_api/v2/db/'; // use `/_api/v2/<db>/_security` endpoint
    ...
    let index = function(definition, callback) {
    ...
    let index_del = function(spec, callback) { // eslint-disable-line camelcase
      spec = spec || {};
      ...
      let type = spec.type || 'json';
      let path = encodeURIComponent(db) + '/_index/' +
    ...
    let find = function(query, callback) {
      return nano.request({path: encodeURIComponent(db) + '/_find',
        method: 'post',
        body: query}, callback);
    };

    let obj = nano._use(db);
    obj.geo = geo;
  ...
  let generate_api_key = function(callback) { // eslint-disable-line camelcase
  ...
  let get_cors = function(callback) { // eslint-disable-line camelcase
  ...
  let set_cors = function(configuration, callback) { // eslint-disable-line camelcase
