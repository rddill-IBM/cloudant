...
module.exports = function(config) {
  ...
  if (config.url) {
    ...
    for (let i = 0; i < cloudantServices.length; i++) {
      if (typeof config.instanceName === 'undefined' || cloudantServices[i].name === config.instanceName) {
        let credentials = cloudantServices[i].credentials;
...
function getOptions(config) {
  ...
  let result = {password: config.password, username: config.key || config.username || config.account};
  return result;
}
