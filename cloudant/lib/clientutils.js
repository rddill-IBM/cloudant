...
let sendResponseToClient = function(response, clientStream, clientCallback) {
...
let updateState = function(state, newState, callback) {
...
let processState = function(r, callback) {
  let abort = function() {
    if (r.response) {
  ...
  if (r.state.abortWithResponse) {
    ...
    let err = new Error('Plugin issued abort');
...
let runHooks = function(hookName, r, data, end) {
  if (r.plugins.length === 0) {
    ...
    async.eachSeries(r.plugins, function(plugin, done) {
      if (typeof plugin[hookName] !== 'function') {
        ...
        let oldState = Object.assign({}, r.state);
...
let wrapCallback = function(r, done) {
