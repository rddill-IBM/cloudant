...
module.exports = {
  ...
  */
  ...
  // let methodName = 'insert';
    let _rdd = this;
    return new Promise(function(resolve, reject) {
      _rdd.helperFind(_name, _object, resolve, reject);
    });
  },

  helperFind(_name, _object, _resolve, _reject) {
    let methodName = 'helperFind';
    let _rdd = this;
    let method = 'POST';
    let url = _rdd.getDBPath() + _name + '/_find';
    delete _object._rev;
    let headers = {};
    ...
    request({
      ...
      if (error) {
        ...
        _reject({error: error});
      } else if ((typeof body !== 'undefined') && ((typeof (body.error) !== 'undefined') && (body.error !== null))) {
        if (body.error === 'too_many_requests') {
          console.log(methodName + ' initial, too many requests, pausing ' + _rdd._rateTimeOut + ' milliseconds.');
          setTimeout(function() {
            _rdd._helperFind(_name, _object, _resolve, _reject);
          }, _rdd._rateTimeOut);
        } else {
          console.log(methodName + ' initial request _body.error: ', body.error);
          _reject({error: body.error + ' ' + body.reason});
        }
      } else {
        _resolve({success: body});
      }
    });
  },
  ...
  */
  insert: function(_name, _object) {
    ...
    let _rdd = this;
    return new Promise(function(resolve, reject) {
      _rdd.helperInsert(_name, _object, resolve, reject);
    });
  },
  helperInsert(_name, _object, _resolve, _reject) {
    let methodName = 'helperInsert';
    let _rdd = this;
    let method = 'POST';
    let url = _rdd.getDBPath() + _name;
    delete _object._rev;
    let headers = {};
    if (_rdd._credentials.useIAM !== null) {
      headers.Authorization = _rdd.cloudantAuth;
    ...
    request({
      ...
      if (error) {
        ...
        if (body.error === 'too_many_requests') {
          console.log(methodName + ' initial, too many requests, pausing ' + _rdd._rateTimeOut + ' milliseconds.');
          setTimeout(function() {
            _rdd._helperInsert(_name, _object, _resolve, _reject);
          }, _rdd._rateTimeOut);
        } else {
          console.log(methodName + ' initial request _body.error: ', body.error);
          _reject({error: body.error + ' ' + body.reason});
  ...
  */
  deleteItem: function(_name, _oid, _rev) {
    ...
    let _rdd = this;
    return new Promise(function(resolve, reject) {
      _rdd.helperDelete(_name, _oid, _rev, resolve, reject);
    });
  },
  helperDelete(_name, _oid, _rev, _resolve, _reject) {
    let methodName = 'helperDelete';
    let _rdd = this;
    let method = 'DELETE';
    let url = _rdd.getDBPath() + _name + '/' + _oid + '?rev=' + _rev;
    let headers = {};
    if (_rdd._credentials.useIAM !== null) {
      headers.Authorization = _rdd.cloudantAuth;
    } else {
    ...
    request({
      ...
      if (error) {
        ...
        if (body.error === 'too_many_requests') {
          console.log(methodName + ' initial, too many requests, pausing ' + _rdd._rateTimeOut + ' milliseconds.');
          setTimeout(function() {
            _rdd._helperDelete(_name, _oid, _rev, _resolve, _reject);
          }, _rdd._rateTimeOut);
        } else {
          console.log(methodName + ' initial request _body.error: ', body.error);
          _reject({error: body.error + ' ' + body.reason});
